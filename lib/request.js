...
Request.prototype.executeRequest = function(route, call, parameters, callback) {
	if (!this.credentials || !this.credentials.access_token || !this.credentials || this.credentials.expires < new Date()) {
		...
		if (parameters && call != 'getMe' && (call == 'getAll' && route != 'companies')) {
		...
		console.log({hostname: this.host, path: this.baseUrl+route+'/'+call+'/?access_token='+this.credentials.access_token, method: method, headers: headers });
